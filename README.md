...
The application is deployed on Render. To enable the AI chatbot feature, you need to add your Gemini
API key:
...
2. **Add the API key to Render:**
    ...
    - Select your `ira-studentwellness` service
...
4. **Verify:**
    ...
    - Login as a student
    - Try the AI chatbot - it should now respond intelligently!

**Note:** Without the API key, the chatbot will work in fallback mode with limited responses.
...
DEBUG=True
GEMINI_API_KEY=your_gemini_api_key_here  # For AI chatbot
```
...
Environment variables are managed in `render.yaml` and Render dashboard:

- `SECRET_KEY`: Auto-generated by Render
- `RENDER`: Automatically set to `true`
- `DISABLE_AI_MODELS`: Set to `true` (to reduce memory usage on free tier)
- `GEMINI_API_KEY`: **Must be set manually in Render dashboard** (see deployment section above)

**Important:** The Gemini API key must be added manually through the Render dashboard for security
reasons.
...
- ✅ Set `GEMINI_API_KEY` in Render dashboard for AI chatbot
- ✅ Ensure `DEBUG=False` (not set in render.yaml, defaults to False in production)
